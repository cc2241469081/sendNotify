...
async function sendNotify(
  ...
  try {
    ...
    if (
      text.indexOf("cookie已失效") != -1 ||
      desp.indexOf("重新登录获取") != -1 ||
      text == "Ninja 运行通知"
    ) {
      if (Notify_CKTask) {
          ...
          if (temptest) {
            if (temptest.status == 0) {
              ...
              if (!isLogin) {
                ...
                strPtPin =
                  strPtPin.match(/pt_pin=([^; ]+)(?=;?)/) &&
                  strPtPin.match(/pt_pin=([^; ]+)(?=;?)/)[1];
                var strAllNotify = "";
                var MessageUserGp2 = "";
                var MessageUserGp3 = "";
                var MessageUserGp4 = "";
                ...
                if (MessageUserGp4) {
                  userIndex4 = MessageUserGp4.findIndex(
                    (item) => item === strPtPin
                  );
                }
                if (MessageUserGp2) {
                  userIndex2 = MessageUserGp2.findIndex(
                    (item) => item === strPtPin
                  );
                }
                if (MessageUserGp3) {
                  userIndex3 = MessageUserGp3.findIndex(
                    (item) => item === strPtPin
                  );
                }
    ...
    if (
      strtext.indexOf("cookie已失效") != -1 ||
      strdesp.indexOf("重新登录获取") != -1 ||
      strtext == "Ninja 运行通知"
    ) {
    ...
    if (
      text.indexOf("任务") != -1 &&
      (text.indexOf("新增") != -1 || text.indexOf("删除") != -1)
    ) {
      strTitle = "脚本任务更新";
    }

    if (strTitle) {
      const notifyRemindList = process.env.NOTIFY_NOREMIND
        ? process.env.NOTIFY_NOREMIND.split("&")
        : [];
    ...
    //检查黑名单屏蔽通知
    const notifySkipList = process.env.NOTIFY_SKIP_LIST
      ? process.env.NOTIFY_SKIP_LIST.split("&")
      : [];
    ...
    for (lncount = 2; lncount < 20; lncount++) {
      if (process.env["NOTIFY_GROUP" + lncount + "_LIST"]) {
        ...
        const titleIndex = notifyGroupList.findIndex(
          (item) => item === strTitle
        );
        if (titleIndex !== -1) {
        ...
        if (strCustomTempArr.length > 1) {
          if (strTitle == strCustomTempArr[0]) {
            ...
            if (strCustomTempArr[1].indexOf("组") != -1) {
              ...
              console.log(
                "自定义设定强制使用组" + UseGroupNotify + "配置通知..."
              );
            } else {
              UseGroupNotify = 1;
            }
    ...
    if (process.env["BARK_PUSH" + UseGroupNotify] && Use_BarkNotify) {
      if (
        process.env["BARK_PUSH" + UseGroupNotify].indexOf("https") > -1 ||
        process.env["BARK_PUSH" + UseGroupNotify].indexOf("http") > -1
      ) {
        //兼容BARK自建用户
        BARK_PUSH = process.env["BARK_PUSH" + UseGroupNotify];
      } else {
        //兼容BARK本地用户只填写设备码的情况
        BARK_PUSH = `https://api.day.app/${
          process.env["BARK_PUSH" + UseGroupNotify]
        }`;
      }
    ...
    const notifySkipRemarkList = process.env.NOTIFY_SKIP_NAMETYPELIST
      ...
      : [];
    const titleIndex3 = notifySkipRemarkList.findIndex(
      (item) => item === strTitle
    );
...
function getQLinfo(strCK, intcreated, strTimestamp, strRemark) {
  var strCheckCK =
    strCK.match(/pt_key=([^; ]+)(?=;?)/) &&
    strCK.match(/pt_key=([^; ]+)(?=;?)/)[1];
  var strPtPin = decodeURIComponent(
    strCK.match(/pt_pin=([^; ]+)(?=;?)/) &&
  ...
  if (strCheckCK.substring(0, 3) == "AAJ") {
    ...
    //过期时间
    var UseDay = Math.ceil(
      (DateToday.getTime() - DateCreated.getTime()) / 86400000
    );
    var LogoutDay =
      30 -
      Math.ceil((DateToday.getTime() - DateTimestamp.getTime()) / 86400000);
    if (LogoutDay < 1) {
      strReturn =
...
async function sendNotifybyWxPucher(
  text,
  desp,
  PtPin,
  author = "\n\n本通知 By ccwav Mod",
